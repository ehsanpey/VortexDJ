...
var commands = {
	...
	"aliases": {
		...
		process: function(bot, msg, suffix) {
		    ...
		    "bing": {
        description: "responds bong, useful for checking if bot is alive",
        process: function(bot, msg, suffix) {
            msg.channel.sendMessage( msg.author+" bong!");
            if(suffix){
			...
			"owner": {
        description: "Calls the owner of this bot for you",
        process: function(bot, msg, suffix) {
            msg.channel.sendMessage( msg.author+" is calling the owner: @CodyJ");
        }
    },
    "idle": {
				usage: "[status]",
    ...
    "offline": {
				usage: "[status]",
        description: "sets bot status to offline",
        process: function(bot,msg,suffix){
        ...
        if(cmdTxt === "help"){
			...
			if(Permissions.checkPermission(msg.author,cmdTxt)){
				try{
					...
					var msgTxt = "command " + cmdTxt + " failed :( try again?";
					if(Config.debug){
						 msgTxt += "\n" + e.stack;
					}
